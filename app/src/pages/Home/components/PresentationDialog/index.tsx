...
const PresentationDialog = injectIntl<PresentationDialogProps>(
  ({ intl, presentation, ...props }) => {
    ...
    const companyName =
      presentation && presentation.company
    ...
    const register = async () => {
      ...
      if (!response || response.status !== 200) {
        enqueueSnackbar(
          intl.formatMessage(messages.errorRegistering),
          errorSnack
        );
        setLoading(false);
        return;
      ...
      enqueueSnackbar(
        intl.formatMessage(messages.successfullyRegistered, {
          company: companyName
        }),
        successSnack
      );
